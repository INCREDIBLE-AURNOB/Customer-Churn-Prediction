...
def main():
    ...
    st.markdown("This app predicts the **Customer Churn**")
    ...
    if add_selectbox=="Online":
        st.info("Please enter the informations")
        ...
        contact=st.selectbox('Contact', ['Month-to-month', 'One year', 'Two year'])
        ...
        data={
            ...
            'InternetService_Fiber_optic': internetservice,
            'OnlineSecurity_No_internet_service': onlinesecurity,
            'OnlineBackup_No_internet_service': onlinebackup,
            'TechSupport_No_internet_service': techsupport,
            'StreamingTV_No_internet_service': streamingtv,
            'StreamingMovies_No_internet_service': streamingmovies,
            'Contract_One_year': contact,
            'Contract_Two_year': contact,
            'PaymentMethod_Electronic_check': paymentmethod
        }
        
        features_df=pd.DataFrame.from_dict([data])
        st.markdown("<h3></h3>", unsafe_allow_html=True)
        st.write('Overview of customer data')
        st.markdown("<h3></h3>", unsafe_allow_html=True)
        st.dataframe(features_df)
        
        #Preprocess  the data
        preprocess_df=process(features_df,'Online')
        ...
        if file is not None:
            ...
            #Preprocess  the data
            preprocess_df=process(data, 'Batch')
